...
class AddOptions extends Component {
    constructor(props) {
        super(props);
        this.state = {
            domains: this.props.selectedRelease && this.props.selectedRelease.TcAggregate 
                    && this.props.selectedRelease.TcAggregate.AvailableDomainOptions && Object.keys(this.props.selectedRelease.TcAggregate.AvailableDomainOptions).join(','),
            selectedDomain: '',
                ...
                <FormGroup row className="my-0" style={{ marginTop: '1rem' }}>
                        ...
                        <FormGroup className='rp-app-table-value'>
                                ...
                                </Label>
                                {
                                    !this.state.isEditing ? 
                                        <div className='rp-app-table-value'><span className='rp-edit-TC-span'>{this.state.domains}</span></div>:
                                        <Input type='select' onChange={(e) => this.setState({selectedDomain: e.target.value, subDomains: this.props.selectedRelease && this.props.selectedRelease.TcAggregate && this.props.selectedRelease.TcAggregate.AvailableDomainOptions[e.target.value] && this.props.selectedRelease.TcAggregate && this.props.selectedRelease.TcAggregate.AvailableDomainOptions[e.target.value].join(',')})} value={this.state.selectedDomain}>
                                            <option value="">Select Domain</option>
                                            {this.props.selectedRelease.TcAggregate && this.props.selectedRelease.TcAggregate.AvailableDomainOptions && 
                                                Object.keys(this.props.selectedRelease.TcAggregate.AvailableDomainOptions).map(item => (
                        ...
                        {
                            !this.state.isEditing ? 
                                <div className='rp-app-table-value'><span className='rp-edit-TC-span'>{}</span></div>:
...
export default connect(mapStateToProps, {updateTCEdit})(AddOptions);